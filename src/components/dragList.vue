...
<template>
  <scroll-view :id="props.id" class="fish-drag-scroll" :scroll-y="true" :scroll-top="state.scrollTop">
    <view class="fish-drag" :style="{ height: `${wrapHeight}px` }">
      ...
      >
        <slot name="item" v-bind:item="item.data" v-bind:index="index" />
        <view
          catch-move
...
import { execSelectQuery } from '@/utils/dom'
...
const onTouchEnd = (event: TouchEvent) => {
  ...
  if (hasChanged) {
    ...
    emit(
      'change',
      listData.sort((a, b) => a.index - b.index).map(i => ({ ...i.data }))
    )
  }
}
